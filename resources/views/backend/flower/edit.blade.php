...
@section('title', 'Edit Flower - Flower Shop Admin')
@section('page-title', 'Edit Flower')
@section('page-description', 'Update the flower information.')

@section('content')
<div class="card shadow mb-4">
    <div class="card-header d-flex align-items-center justify-content-between py-3">
        <h6 class="m-0 font-weight-bold text-primary">Edit Flower: {{ $flower->name }}</h6>
    ...
    <div class="card-body">
        <form action="{{route('admin.flowers.update',$flower->id)}}" method="POST" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="row mb-3">
                ...
                <div class="col-sm-10">
                    <input type="text" name="flowerName" class="form-control w-50 @error('flowerName') is-invalid @enderror"
                           value="{{old('flowerName', $flower->name)}}" id="name" placeholder="Enter flower name">
                    @error('flowerName')
            ...
            <div class="row mb-3">
                ...
                <div class="col-sm-10">
                    <input type="number" name="flowerPrice" class="form-control w-50 @error('flowerPrice') is-invalid @enderror"
                           id="price" value="{{old('flowerPrice', $flower->price)}}" placeholder="Enter price">
                    @error('flowerPrice')
                        <span class="invalid-feedback" role="alert">
                            <strong>{{ $message }}</strong>
                        </span>
                    ...
                    <select name="flowerType" class="form-select form-control w-50 @error('flowerType') is-invalid @enderror" id="type">
                        ...
                        @foreach ($types as $type)
                            <option value="{{$type->id}}"
                                {{old('flowerType', $flower->type_id) == $type->id ? 'selected' : ''}}>{{$type->name}}</option>
                        @endforeach
                    </select>
                    @error('flowerType')
                        <span class="invalid-feedback" role="alert">
                    ...
                    <select name="flowerCategory" class="form-select form-control w-50 @error('flowerCategory') is-invalid @enderror" id="category">
                        ...
                        @foreach ($categories as $category)
                            <option value="{{$category->id}}"
                                {{old('flowerCategory', $flower->category_id) == $category->id ? 'selected' : ''}}>{{$category->name}}</option>
                        @endforeach
                    </select>
            ...
            <div class="row mb-3">
                ...
                <div class="col-sm-10">
                    ...
                    @if($flower->image)
                        <div class="mt-2">
                            <img src="{{asset($flower->image)}}" alt="Current image" style="max-width: 200px; height: auto;" class="rounded">
            ...
            <div class="row mb-3">
                ...
                <div class="col-sm-10">
                    <textarea name="flowerDescription" rows="5" class="form-control w-50 @error('flowerDescription') is-invalid @enderror"
                              id="description" placeholder="Enter flower description">{{old('flowerDescription', $flower->description)}}</textarea>
            ...
            <div class="row">
                <div class="offset-sm-2 col-sm-10">
                    <button type="submit" class="btn btn-primary">
                        <i class="fas fa-save mr-2"></i>Update Flower
                    </button>
